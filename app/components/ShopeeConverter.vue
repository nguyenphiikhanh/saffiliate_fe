<template>
  <div class="w-full animate-fade-in-up">
    ...
    <div 
      ...
      :class="currentType === AFFILIATE_TYPES.SHOPEE ? 'glow-orange' : 'glow-tiktok'"
    >
      <!-- Premium Creative Brand Selection Grid -->
      <div class="mb-8 select-none">
        <div class="flex items-center gap-2 mb-4">
          <div 
            class="w-6 h-6 rounded-full font-bold text-xs flex items-center justify-center border shrink-0 transition-all duration-300"
            :class="currentType === AFFILIATE_TYPES.SHOPEE 
              ? 'bg-orange-500/10 dark:bg-orange-500/20 text-shopee-orange border-orange-500/10' 
              : 'bg-cyan-500/10 dark:bg-cyan-500/20 text-cyan-500 border-cyan-500/10'"
          >
            1
          </div>
          <h3 class="text-xs font-black text-slate-700 dark:text-slate-350 uppercase tracking-widest">
            Chọn Kênh Quy Đổi
          </h3>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 max-w-md mx-auto">
          ...
          >
            ...
            <div 
              v-if="currentType === AFFILIATE_TYPES.SHOPEE"
              class="absolute -right-8 -bottom-8 w-20 h-20 bg-shopee-orange/5 dark:bg-shopee-orange/10 rounded-full blur-xl pointer-events-none"
            ></div>
            ...
            <div>
              <div 
                ...
                :class="currentType === AFFILIATE_TYPES.SHOPEE ? 'text-shopee-orange' : 'text-slate-400'"
              >
                Hoàn tiền
              </div>
              <div 
                class="text-sm sm:text-base font-extrabold tracking-tight mt-0.5"
                :class="currentType === AFFILIATE_TYPES.SHOPEE ? 'text-slate-900 dark:text-white' : 'text-slate-600 dark:text-slate-400'"
              >
                Shopee
              </div>
            </div>
          ...
          >
            <!-- Decorative gradient blur -->
            <div 
              v-if="currentType === AFFILIATE_TYPES.TIKTOK"
              class="absolute -right-8 -bottom-8 w-20 h-20 bg-cyan-500/5 dark:bg-cyan-500/10 rounded-full blur-xl pointer-events-none"
            ></div>
            ...
            <div>
              <div 
                class="text-[9px] font-black tracking-wider uppercase transition-colors"
                :class="currentType === AFFILIATE_TYPES.TIKTOK ? 'text-slate-950 dark:text-cyan-400' : 'text-slate-400'"
              >
                Hoàn tiền
              </div>
              <div 
                class="text-sm sm:text-base font-extrabold tracking-tight mt-0.5"
                :class="currentType === AFFILIATE_TYPES.TIKTOK ? 'text-slate-950 dark:text-white' : 'text-slate-600 dark:text-slate-400'"
              >
                TikTok
              </div>
            </div>
      ...
      <div class="flex justify-center mb-4 select-none">
        <div class="relative group">
          ...
          <div class="absolute inset-0 bg-amber-500/10 rounded-full blur-xl scale-95 group-hover:scale-110 transition-all duration-700 animate-pulse"></div>
      ...
      <div class="text-center mb-8">
        ...
        <p class="text-slate-600 dark:text-slate-400 text-sm mt-2 font-medium transition-colors duration-400">
          Dán link sản phẩm 
          <span class="font-bold">{{ currentType === AFFILIATE_TYPES.SHOPEE ? 'Shopee' : 'TikTok' }}</span> 
      ...
      <div class="space-y-5">
        <div class="relative">
          ...
          <input
            ...
            :class="{
              'border-shopee-orange/50': isValidating && isUrlValid && currentType === AFFILIATE_TYPES.SHOPEE,
              'border-cyan-500/50': isValidating && isUrlValid && currentType === AFFILIATE_TYPES.TIKTOK,
        ...
        <button
          ...
          :class="[
            isLoading || !rawUrl
              ...
              : currentType === AFFILIATE_TYPES.SHOPEE
                ? 'bg-shopee-orange hover:bg-shopee-orange-hover border-shopee-orange text-white active:scale-98 cursor-pointer glow-orange-button'
                : 'bg-slate-900 hover:bg-slate-950 dark:bg-slate-850 dark:hover:bg-slate-800 border-slate-900 dark:border-slate-850 text-white hover:text-cyan-400 dark:hover:text-cyan-400 active:scale-98 cursor-pointer glow-tiktok-button'
          ]"
        >
          <!-- Loading Spinner -->
    ...
    <ShopeeResult v-if="resultLink && !isLoading" :link="resultLink" :affiliate-link="affiliateLink" :product-info="productInfo" :platform-type="currentType" @reset="handleCloseModal" />
  </div>
</template>
...
const selectType = (type) => {
  ...
  handleClear(); // Xóa sạch link cũ và reset trạng thái quy đổi trước đó
  nextTick(() => {
    urlInput.value?.focus(); // Tự động focus vào ô nhập link
  });
};
</script>
...
.glow-tiktok {
  box-shadow: 0 10px 40px -10px rgba(6, 182, 212, 0.12);
