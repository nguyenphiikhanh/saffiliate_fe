  ...
  <div class="flex flex-col gap-6 pb-12">
    ...
    <a-card :bordered="false" class="admin-card" :body-style="{ padding: 0 }">
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-if="column.key === 'name'">
            <div class="flex items-center gap-3 py-1">
              <div
                class="h-9 w-9 rounded-full overflow-hidden flex items-center justify-center bg-slate-100 dark:bg-slate-800 shrink-0 border border-slate-200 dark:border-slate-700"
              >
                <img
                  v-if="record.image"
                  :src="record.image"
                ...
                <span
                  v-else
                  class="text-slate-500 font-bold text-xs uppercase"
              ...
              <div class="min-w-0">
                <div class="flex items-center gap-1.5">
                  <span
                    ...
                    >{{ record.name || "User" }}</span
                  >
                  <a-tag
                    v-if="isNewUser(record.createdAt)"
                    color="green"
                ...
                <span
                  class="text-[10px] text-slate-400 block mt-0.5 truncate max-w-[200px] select-all"
                  >{{ record.email }}</span
                >
              </div>
            </div>
          ...
          <template v-else-if="column.key === 'rank'">
            <div class="flex items-center gap-1.5">
              <img
                :src="getRankStyles(record.rank).image"
                class="h-4 w-4 object-contain"
                :alt="record.rank"
              />
              <a-tag :color="getRankTagColor(record.rank)">{{
                getRankStyles(record.rank).name
              }}</a-tag>
            </div>
          </template>
          ...
          <template v-else-if="column.key === 'availableBalance'">
            <span
              class="font-black text-emerald-600 dark:text-emerald-400 text-xs"
              >{{ formatCurrency(record.availableBalance) }}</span
            >
          </template>
          ...
          <template v-else-if="column.key === 'completedOrdersCount'">
            <span
              class="font-bold text-slate-700 dark:text-slate-300 text-xs"
              >{{ record.completedOrdersCount }}</span
            >
          </template>
      ...
      <div
        ...
        class="px-4 py-3 flex justify-end border-t border-slate-100 dark:border-slate-800"
      >
        <a-pagination
    ...
    >
      <div v-if="selectedUser" class="flex flex-col gap-6">
        <p class="text-[11px] text-slate-500 font-medium select-all -mt-4 mb-0">
          UID: {{ selectedUser.id }}
        </p>
        ...
        <div class="flex flex-col items-center text-center gap-3">
          <div
            class="h-20 w-20 rounded-full overflow-hidden flex items-center justify-center bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700"
          >
            <img
              ...
              referrerpolicy="no-referrer"
            />
            <span v-else class="text-slate-500 font-bold text-2xl uppercase">{{
          ...
          <div>
            ...
            <p class="text-sm text-slate-500 select-all">
              {{ selectedUser.email }}
            </p>
            <a-tag
          ...
          <a-descriptions-item label="Cấp bậc">
            <div class="flex items-center gap-1.5">
              ...
              <span class="font-bold text-xs">{{
                getRankStyles(selectedUser.rank).fullName
              }}</span>
            </div>
          </a-descriptions-item>
            ...
            <span class="text-xs">
              {{
                selectedUser.ordersToNextRank > 0
                  ? `Cần thêm ${selectedUser.ordersToNextRank} đơn`
                  : "Đã đạt cấp tối đa"
              }}
            </span>
          </a-descriptions-item>
...
useHead({ title: "Quản lý Thành Viên | Saffi Admin" });
