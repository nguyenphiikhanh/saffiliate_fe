<template>
  <div class="w-full pb-24 md:pb-6">
    <a-collapse
    ...
    >
      <!-- Important Notes -->
      <a-collapse-panel key="1" class="mb-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <template #header>
          <div class="flex items-center gap-3 py-1">
            <div class="w-10 h-10 rounded-xl bg-amber-50 dark:bg-amber-900/20 text-amber-500 flex items-center justify-center shrink-0">
              <WarningOutlined class="text-xl" />
            </div>
            <div class="flex flex-col text-left">
              <span class="font-bold text-[13px] text-slate-800 dark:text-slate-100 uppercase tracking-wider">Lưu ý Quan trọng</span>
              <span class="text-[11px] font-bold text-slate-500 mt-0.5">Đọc kỹ để đảm bảo nhận hoàn tiền mua sắm</span>
            </div>
          </div>
        </template>
        <template #extra>
        ...
        <div class="px-2 pb-2">
          <div class="flex flex-col gap-3 mb-6">
            <div class="flex items-start gap-3" v-for="(note, idx) in importantNotes" :key="idx">
              <div class="w-5 h-5 rounded-full bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-500 font-black text-[10px] flex items-center justify-center shrink-0 mt-0.5 border border-amber-100 dark:border-amber-800">
                {{ idx + 1 }}
              </div>
              <p class="text-[13px] text-slate-600 dark:text-slate-400 font-medium leading-relaxed m-0" v-html="note"></p>
            </div>
          </div>
      ...
      <!-- Quick Guide Card -->
      <a-collapse-panel key="2" class="mb-4 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl shadow-sm overflow-hidden">
        <template #header>
          <div class="flex items-center gap-3 py-1">
            <div class="w-10 h-10 rounded-xl bg-orange-50 dark:bg-orange-900/20 text-[#ee4d2d] flex items-center justify-center shrink-0">
              <BookOutlined class="text-xl" />
            </div>
            <div class="flex flex-col text-left">
              <span class="font-bold text-[13px] text-slate-800 dark:text-slate-100 uppercase tracking-wider">Hướng Dẫn Nhanh</span>
              <span class="text-[11px] font-bold text-slate-500 mt-0.5">Cách mua hàng qua Saffi & nhận hoàn tiền</span>
            </div>
          </div>
        </template>
        <template #extra>
          <span class="text-xs font-bold w-8 text-center select-none text-[#ee4d2d] mr-1">{{ activeKey.includes('2') ? 'Ẩn' : 'Xem' }}</span>
        </template>
        
        <div class="px-2 pb-2">
          <a-carousel arrows class="bg-slate-50 dark:bg-slate-800 border border-dashed border-slate-200 dark:border-slate-700 rounded-xl mb-4">
            <template #prevArrow>
            ...
            <div v-for="(step, index) in steps" :key="'step-'+index">
              <div class="pt-6 pb-10 px-8 flex flex-col items-center text-center min-h-[260px] select-none">
                <div v-if="step.image" class="mb-4 rounded-xl overflow-hidden shadow-sm inline-block max-w-[200px] border border-slate-100 dark:border-slate-700">
                  <img :src="step.image" :alt="step.title" class="w-full h-auto object-cover pointer-events-none" />
                </div>
                <h5 class="text-sm font-bold text-slate-800 dark:text-slate-200 mb-2">{{ step.title }}</h5>
                <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed mb-0" v-html="step.desc"></p>
              </div>
            </div>
          </a-carousel>
          
          <div class="flex justify-center mt-2">
            <a-button type="primary" size="small" @click="activeKey = activeKey.filter(k => k !== '2')" class="px-6 font-bold rounded-full">
              Đã hiểu
            </a-button>
          </div>
        </div>
...
const steps = [
  {
    ...
    desc: "Mở ứng dụng Shopee hoặc TikTok, chọn sản phẩm cần mua và nhấn nút <span class='text-[#ee4d2d] font-bold bg-orange-50 px-1 rounded'>Chia sẻ</span> (hình mũi tên).",
    image: "/step/1.jpg",
  },
  {
