<template>
  <a-layout class="h-screen overflow-hidden">
    ...
    >
      <!-- Logo -->
      <div
        class="h-16 flex items-center justify-center border-b border-slate-200 dark:border-slate-800 px-4"
      >
        <NuxtLink
          to="/admin"
          class="flex items-center gap-2 cursor-pointer w-full overflow-hidden"
        >
          <div
            class="w-8 h-8 rounded-lg bg-slate-900 dark:bg-slate-100 flex items-center justify-center text-white dark:text-slate-900 shrink-0"
          >
            <span class="font-black text-lg leading-none">S</span>
          </div>
          <div class="flex flex-col" v-show="!collapsed">
            <span
              class="font-bold text-[15px] leading-tight tracking-tight text-slate-800 dark:text-slate-100"
              >Saffi</span
            >
            <span
    ...
    <a-layout>
      ...
      >
        ...
        <div class="flex items-center gap-3 select-none">
          ...
          <span
            class="text-xs font-semibold text-slate-600 dark:text-slate-400 hidden sm:inline"
            >{{ userName }}</span
          >
          <div
            class="h-8 w-8 rounded-full overflow-hidden flex items-center justify-center border border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-800 shrink-0"
          >
            <img
              v-if="userAvatar"
              :src="userAvatar"
            ...
            <div
              v-else
              class="h-full w-full bg-slate-800 dark:bg-slate-100 text-white dark:text-slate-900 font-bold text-xs flex items-center justify-center uppercase"
            >
              {{ firstLetter }}
            </div>
          </div>
